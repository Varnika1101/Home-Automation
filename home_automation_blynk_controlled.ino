...
#define BLYNK_TEMPLATE_ID "TMPL3N3Jrr-u"
#define BLYNK_DEVICE_NAME "Home automation"
#define BLYNK_AUTH_TOKEN "6qugCZ8HBcUlOUHOQM9w1kUY8NM6NTs_"
...
void handle_temp(void)
{
  if(read_temperature() > float(35) && heater_sw)
  {
    ...
    lcd.setCursor(7,0);
    lcd.print("HT_R OFF");
    // send notification on Blynk IOT app
   ...
   if ((read_temperature() < float(28)) && cooler_sw)
  {
    cooler_sw = 0;
    cooler_control(OFF);
    lcd.setCursor(7, 0);
...
void handle_tank(void)
{
  ...
  if((tank_volume<2000) && (inlet_sw == OFF))
  {
    ...
    Blynk.virtualWrite(BLYNK_TERMINAL_V_PIN, "Wter inflow enabled\n");
  ...
  if((tank_volume == 3000)&& (inlet_sw == ON))
  {
    ...
    Blynk.virtualWrite(BLYNK_TERMINAL_V_PIN, "Wter inflow disabled\n");
...
void setup(void)
{
    ...
    //display volume
     lcd.setCursor(0,1);
    lcd.print("V=");
    // initialisation of temp system
    init_temperature_system();
    init_ldr();
    // initialisation of Serial Tank
    init_serial_tank();
...
void loop(void) 
{
  // to run blynk related function
   Blynk.run();
   // to call set interval at particular period
   timer.run();
   // read temperature and display it on the dashboard
   String temperature;
   temperature=String (read_temperature(), 2); //e.g. 27.37
   lcd.setCursor(2,0);
   ...
   brightness_control();
   // to control threshhold temperature of 35 degree
   handle_temp();
   // to control volume of water in the tank
   handle_tank();
}
